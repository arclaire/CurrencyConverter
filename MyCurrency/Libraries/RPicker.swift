...
let pickerAnimationDuration: TimeInterval = 0.03
...
class RPicker: NSObject {
    ...
    class func selectDate(title: String = "",
        ...
        if let currentController = UIWindow.currentController {
            ...
            let tapGestureRecognizer =  UITapGestureRecognizer(target: toolBar, action: #selector(RToolBar.cancelAction))
            viewTransperant.addGestureRecognizer(tapGestureRecognizer)
...
class RToolBar: UIView {
    ...
    open var toolbar: UIToolbar {
		...
		let toolBarTitleItem = toolBarL.titleItem(text: "", font: UIFont.systemFont(ofSize: 12), color: UIColor.blue)
